    ...
    <section style={{ background: '#fff', padding: 'clamp(3rem, 6vw, 5rem) 1.25rem' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto', width: '100%' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(3rem, 7vw, 5.5rem)' }}>
          {BENEFITS.map((b, i) => (
            <Reveal key={b.title} variant={i % 2 === 1 ? 'right' : 'left'} className={`bd-row${i % 2 === 1 ? ' bd-row--rev' : ''}`}>
              {/* 텍스트 */}
    ...
    >
      <div style={{ maxWidth: '1100px', margin: '0 auto', width: '100%' }}>
        ...
        <SplitText
          as="h3"
          className="title-1 bd-heading"
          style={{ margin: '0.75rem 0 0', color: '#fff', wordBreak: 'keep-all', lineHeight: 1.4 }}
          step={0.024}
          segments={[
            { text: '모두가 잠든 이 시간에도,\n위플로우는 ' },
        ...
        <Reveal variant="up">
          <Link href="/diagnosis" className="btn-white" style={{ marginTop: '3rem', fontSize: '1rem' }}>
            무료진단 신청하기
          </Link>
        </Reveal>
      </div>
