...
import { ArrowUpRight, Dumbbell, Landmark, Scale, Car, Heart, Home, Coffee, Scissors, Building2, PawPrint, Sofa, Wrench, Gamepad2, GraduationCap, type LucideIcon } from 'lucide-react'
...
const ALL_CATEGORIES = ['전체', ...Array.from(new Set(cases.map(c => c.category)))]
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        padding: '0.5rem 0.7rem', display: 'flex', alignItems: 'center', gap: '0.5rem',
      }}>
        ...
        <div style={{
          flex: 1, background: '#e5e7eb', borderRadius: '5px',
          padding: '0.2rem 0.6rem', fontSize: '0.66rem', color: '#a09890',
      ...
      {/* 이미지 */}
      <div style={{ position: 'relative', height: '190px', overflow: 'hidden', background: c.color + '22' }}>
        <Image
          src={caseImagePath(c.slug)} alt={c.name} fill sizes="400px"
          style={{
            objectFit: 'cover',
        ...
        {/* 카테고리 뱃지 */}
        <span className="caption-2 emphasized c-primary" style={{
      ...
      <div style={{ padding: '0.95rem 1.1rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.45rem', marginBottom: '0.25rem' }}>
          {(() => { const Icon = CATEGORY_ICONS[c.category]; return Icon ? <Icon size={14} color={c.color} strokeWidth={2} style={{ flexShrink: 0 }} /> : null })()}
          <p className="subhead emphasized c-primary" style={{ margin: 0 }}>{c.name}</p>
        </div>
        <p className="footnote semibold" style={{
...
function GridReveal({ filtered }: { filtered: typeof cases }) {
  ...
  return (
    <div ref={ref} className="cases-grid">
      {filtered.map((c, i) => <CaseCard key={c.slug} c={c} index={i} visible={visible} />)}
    </div>
  )
}
...
export default function CasesPage() {
  ...
  if (cases.length === 0) {
    return (
      <div>
        <section style={{
          background: '#fff', padding: 'clamp(4rem, 10vw, 7rem) 1.5rem', textAlign: 'center',
          minHeight: '55vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
        }}>
          <p className="caption-2 emphasized c-accent" style={{ letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '0.6rem' }}>SUCCESS CASES</p>
          <h1 className="title-1" style={{ margin: '0 0 0.9rem' }}>업종별 성공사례</h1>
          <p className="callout c-muted" style={{ margin: '0 0 1.75rem', maxWidth: '440px', wordBreak: 'keep-all' }}>
            포트폴리오를 준비 중입니다. 곧 실제 제작 사례로 찾아뵐게요.
          </p>
  ...
  return (
    <div>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
            ...
            <div style={{
              ...
              {[
                ...
                <div key={label} style={{
                  background: '#f3f4f6', borderRadius: '10px', padding: '0.5rem 0.9rem', textAlign: 'center',
                }}>
                  <p className="body emphasized c-accent" style={{ margin: 0, letterSpacing: '-0.02em' }}>{num}</p>
                  <p className="caption-2 semibold c-muted" style={{ margin: 0 }}>{label}</p>
                </div>
              ))}
            </div>
          </div>
          <p className="callout c-muted" style={{
            maxWidth: '480px', margin: '0.75rem 0 0',
            opacity: mounted ? 1 : 0, transition: 'opacity 0.4s ease 0.18s',
          }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {ALL_CATEGORIES.map(cat => {
            ...
            const count = cat === '전체' ? cases.length : cases.filter(c => c.category === cat).length
            return (
      ...
      <section style={{ padding: '2rem 1.5rem 4rem', background: '#fff' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <p className="caption-2 emphasized c-accent" style={{
              letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 0.4rem',
            }}>CASE DETAILS</p>
            <h2 className="title-2 emphasized" style={{ margin: '0 0 0.5rem', wordBreak: 'keep-all' }}>
              {activeCategory === '전체' ? '업종별 성공사례 상세' : `${activeCategory} 성공사례 상세`}
            </h2>
      ...
      {/* ── 더보기 ── */}
      <section style={{ padding: '2.5rem 1.5rem 4rem', background: '#fff', textAlign: 'center' }}>
        <Link
          href="/diagnosis"
