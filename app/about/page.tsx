...
export default function AboutPage() {
  return (
    <main style={{ background: '#fff' }}>
      {/* 인트로 */}
      <section style={{ padding: 'clamp(3.5rem, 7vw, 6rem) 1.25rem', borderBottom: '1px solid var(--border)' }}>
        <div className="about-hero">
          ...
          <Reveal variant="up" delay={0.15}>
            <p className="title-3 c-muted" style={{ margin: '1rem 0 0', letterSpacing: '0.01em' }}>
              People move. Technology follows.
            </p>
            <p className="about-hero-body c-secondary" style={{ margin: '1.75rem 0 0', maxWidth: '720px', wordBreak: 'keep-all', lineHeight: 1.8 }}>
              WEFLOW는 사람과 기술이 함께 흘러가며 더 좋은 방향을 만드는 회사입니다.
              단순히 개발만 하는 회사가 아니라, 기술은 뒤에서 받쳐주고 사람은 앞에서 빛나게 하는 흐름을 만듭니다.
            </p>
          </Reveal>
          <Reveal variant="up" delay={0.1}>
            <div className="about-img" style={{ aspectRatio: '16 / 9', marginTop: 'clamp(2rem, 4vw, 3rem)' }}>이미지</div>
          </Reveal>
        </div>
      </section>
      ...
      <section style={{ padding: 'clamp(3rem, 7vw, 5rem) 1.25rem' }}>
        <div className="about-story">
          ...
          <Reveal as="div" stagger style={{ marginTop: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
            {STORY.map((line) => (
              <p key={line} className="c-secondary" style={{ margin: 0, wordBreak: 'keep-all', fontSize: 'clamp(1.2rem, 2.6vw, 1.5rem)', lineHeight: 1.85 }}>
                {line}
              </p>
            ))}
      ...
      <section style={{ padding: 'clamp(3.5rem, 7vw, 5.5rem) 1.25rem', background: 'var(--accent-dim)', textAlign: 'center' }}>
        <Reveal variant="zoom">
          <p className="emphasized" style={{ margin: 0, color: '#fff', wordBreak: 'keep-all', fontSize: 'clamp(2rem, 5vw, 3.25rem)', lineHeight: 1.25 }}>
            Flow Together, Grow Beyond.
          </p>
          <p style={{ margin: '0.9rem 0 2rem', color: '#9dbff6', fontSize: 'clamp(1.15rem, 2.6vw, 1.4rem)' }}>
            함께 흐르고, 더 크게 성장하다
          </p>
          <Link href="/diagnosis" className="btn-white" style={{ fontSize: '1.15rem', padding: '0.9rem 2rem' }}>
            무료진단 신청하기
          </Link>
        </Reveal>
      </section>
