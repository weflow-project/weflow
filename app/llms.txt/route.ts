...
const planLines = makePlans
  .map(
    p =>
      ...
      `- ${p.sub} (${p.name}): 가격·월 유지보수·관리자 페이지 옵션 모두 상담 문의 · ${p.note}`,
  )
  .join('\n')
...
- [WEFLOW만의 차별점](${BASE}/difference): 템플릿 제작 업체와 최신 기술(React·Next.js)로 직접 만드는 WEFLOW의 차이 — 기능 제약·보안·속도·검색 노출·디자인
